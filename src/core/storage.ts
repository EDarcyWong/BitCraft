import { parseCircuit, type Circuit } from './circuit'
...
export function parseProject(text: string): Project {
  ...
  if (!record(value) || value.format !== 'bitcraft-circuit' || value.version !== 1)
    throw new Error('不支持这个作品格式，请导入比特工坊导出的 v1 文件。')
  return { name: projectName(value.name), circuit: parseCircuit(value.circuit) }
}
export function exportProject(project: Project): string {
  return JSON.stringify(
    { format: 'bitcraft-circuit', version: 1, name: project.name, circuit: project.circuit },
    null,
    2,
  )
}
